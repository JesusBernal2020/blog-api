...
exports.createPost = catchAsync(async (req, res, next) => {
  ...
  const postImgsPromises = req.files.map(async (file) => {
    const imgRef = ref(storage, `posts/${crypto.randomUUID()}-${file.originalname}`);
...
exports.findOnePost = catchAsync(async (req, res, next) => {
  ...
  if (post.comments?.length > 0) {
    userImgsCommentsPromises = post.comments.map(async (comment) => { 
  ...
  const arrPromises = [...postImgsPromises, ...userImgsCommentsPromises];
  /*  await Promise.all(userImgsCommentsPromises); */
...
exports.deletePost = catchAsync(async (req, res, next) => {
  const { post } = req;
  const postDelete = await post.update({ status: postStatus.inactive });
  return res.status(200).json({
    status: 'success',
